  ...
  return (
    <div className="flex h-full flex-col bg-holio-offwhite dark:bg-gray-950">
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <button onClick={openCreate} className="flex items-center gap-1 text-sm font-medium text-holio-orange">
      ...
      <div className="flex-1 overflow-y-auto pb-8">
        ...
        {customFolders.length === 0 ? (
          <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900 px-4 py-8 text-center">
            ...
            <button onClick={openCreate} className="mt-3 text-sm font-medium text-holio-orange">Create one</button>
          </div>
        ) : (
          <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900">
            {customFolders.map((folder, i) => (
              <div key={folder.id}>
                {i > 0 && <div className="mx-4 border-t border-gray-100 dark:border-gray-800" />}
                <div className="flex items-center gap-3 px-4 py-3">
                  <FolderOpen className="h-5 w-5 text-holio-orange" />
                  ...
                  <button onClick={() => openEdit(folder)} className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800">
                    <Pencil className="h-3.5 w-3.5 text-holio-muted" />
                  </button>
                  <button onClick={() => setDeleteConfirm(folder.id)} className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-red-50 dark:hover:bg-red-950">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-xl">
            ...
            <input
              ...
              className="mb-4 w-full rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 text-sm text-holio-text outline-none focus:ring-2 focus:ring-holio-orange dark:bg-gray-800"
              autoFocus
            />
            ...
            <div className="flex gap-2">
              <button onClick={() => setDialogOpen(false)} className="flex-1 rounded-xl bg-gray-100 dark:bg-gray-800 py-2.5 text-sm font-medium text-holio-text hover:bg-gray-200 dark:hover:bg-gray-700">Cancel</button>
              <button onClick={handleSave} disabled={!name.trim()} className="flex-1 rounded-xl bg-holio-orange py-2.5 text-sm font-medium text-white hover:bg-orange-500 disabled:opacity-50">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-xl">
            ...
            <div className="flex gap-2">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 rounded-xl bg-gray-100 dark:bg-gray-800 py-2.5 text-sm font-medium text-holio-text hover:bg-gray-200 dark:hover:bg-gray-700">Cancel</button>
