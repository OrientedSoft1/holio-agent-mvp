      ...
      <div className="flex-1 overflow-y-auto pb-8">
        ...
        <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900 p-5">
          <div className="mb-4 flex flex-wrap gap-3">
            {BG_COLORS.map((c) => (
              <button key={c.id} onClick={() => setBgColor(c.id)} className={cn('flex h-10 w-10 items-center justify-center rounded-full border-2 transition-colors', bgColor === c.id ? 'border-holio-orange' : 'border-gray-200 dark:border-gray-700')} style={{ backgroundColor: c.value }}>
                {bgColor === c.id && <svg className="h-4 w-4 text-holio-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
              </button>
            ))}
          </div>
        ...
        <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900 p-5">
          <div className="flex items-center gap-3">
            ...
            <input type="range" min={12} max={20} value={textSize} onChange={(e) => setTextSize(Number(e.target.value))} className="flex-1 accent-holio-orange" />
        ...
        <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <button onClick={() => setSendByEnter(!sendByEnter)} className={cn('relative h-6 w-11 rounded-full', sendByEnter ? 'bg-holio-orange' : 'bg-gray-300 dark:bg-gray-600')}>
          ...
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <button onClick={() => setRaiseToListen(!raiseToListen)} className={cn('relative h-6 w-11 rounded-full', raiseToListen ? 'bg-holio-orange' : 'bg-gray-300 dark:bg-gray-600')}>
