...
export default function ContactsPanel({ onStartChat }: ContactsPanelProps) {
  ...
  const createDM = useChatStore((s) => s.createDM); const onlineUsers = usePresenceStore((s) => s.onlineUsers); const lsMap = usePresenceStore((s) => s.lastSeen)
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div ref={ref} className="flex-1 overflow-y-auto">
        <button onClick={() => navigate('/invite-friends')} className="flex w-full items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-50"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-50"><Share2 className="h-5 w-5 text-blue-500" /></div><span className="text-sm font-medium text-blue-500">Invite Friends</span></button>
        ...
        {grouped.map(({ letter, items }) => (<div key={letter}><div className="sticky top-0 z-[5] bg-gray-50 px-4 py-1"><span className="text-xs font-semibold text-holio-muted">{letter}</span></div>{items.map((ct) => { const u = ct.contactUser; const nm = dn(ct); const ini = nm.split(' ').filter(Boolean).map((w) => w[0]).join('').slice(0, 2).toUpperCase(); return (<button key={ct.id} onClick={() => msg(ct)} className="flex w-full items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gray-50"><div className="relative flex-shrink-0">{u.avatarUrl ? <img src={u.avatarUrl} alt={nm} className="h-12 w-12 rounded-full object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-full bg-holio-lavender/30 text-sm font-semibold text-holio-text">{ini}</div>}{onlineUsers[ct.contactUserId] && <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-green-500" />}</div><div className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-medium text-holio-text">{nm}</p><p className="truncate text-xs text-holio-muted">{onlineUsers[ct.contactUserId] ? 'online' : lsMap[ct.contactUserId] ? `last seen ${fls(lsMap[ct.contactUserId])}` : 'last seen recently'}</p></div></button>) })}</div>))}
      </div>
